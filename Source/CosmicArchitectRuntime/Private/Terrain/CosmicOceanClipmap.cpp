...
UCosmicOceanClipmap::UCosmicOceanClipmap()
{
	bTickInEditor = true;
...
void UCosmicOceanClipmap::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void UCosmicOceanClipmap::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UCosmicOceanClipmap::CreateLevels()
{
    if (bInit)
    {
        ClearLevels();
...
void UCosmicOceanClipmap::CreatePerformanceLevel(bool bActive)
{
    if (FarLevel) return;
...
void UCosmicOceanClipmap::ClearLevels()
{
    bInit = false;
...
void UCosmicOceanClipmap::ReasignLevels()
{
...
void UCosmicOceanClipmap::UpdatePatchTransform(const FVector& SurfacePos, const FVector& N)
{
    const FVector Up = N;
...
FVector UCosmicOceanClipmap::GetPlayerLocation()
{
...
float UCosmicOceanClipmap::GetDistanceToSurface(FVector& SurfacePos, FVector& N)
{
...
bool UCosmicOceanClipmap::IsClipmapRingVisible(const int32 LevelIndex, const float DistanceToSurface)
{
...
bool UCosmicOceanClipmap::IsClipmapRingVisible(const float GridSpacing, const int32 Resolution, const float DistanceToSurface)
{
...
void UCosmicOceanClipmap::ReduceClimapLevel()
{
    if (NumLevels > 1)
    {
...
void UCosmicOceanClipmap::IncreaseClipmapLevel()
{
    if (NumLevels > 1)
    {
        float spacing = Levels[0]->GridSpacing;
